...
def uncompress(mult, data):
    ...
    for i in range(0, char_to_repeat.count('(')):
        ...
        # get characters to repeat (num_chars after parenthesis)
        char_to_repeat = data[marker_2+1:marker_2+1+num_chars]
        return uncompress(num_times, char_to_repeat)
    '''
    print num_times, 'x', char_to_repeat
    
    print int(char_to_repeat.count('('))
    print
    
    # continue searching for markers in the leftovers
    #while ('(' in char_to_repeat):
    for i in range(0, int(char_to_repeat.count('('))):
        print i
        return uncompress(num_times, char_to_repeat)

    else:
        return (num_times * len(char_to_repeat))

# can't actually perform decompression
# need to reverse engineer the algorithm to determine how it should work
total_length = 0
...
while ('(' in example):
    ...
    # get anything before first marker
    total_length = total_length + marker_1
    
    total_length = total_length + uncompress(1, example[marker_1:])
    
    example = example[marker_2+1+num_chars:]
    
    # if no markers left, just add remaining characters
    if ('(' not in example):
        total_length = total_length + len(example)
